...
//    маска наследуется и может маскировать биты прав доступа
    umask(0);
...
//    Закрыть все ненужные открытые файловые дескрипторы
//    которые процесс-демон может унаследовать и препятствовать их закрытию
...
//    позволяет вызывать фунции стандартного ввода вывода, не получая при этом ошибок
